# -*- coding: utf-8 -*-
# test.py — 저장된 모델 입력 자동 매핑 + 추론단 뉴스 비중 강화(학습 로직/경로 그대로)
...
# 추론 시 숫자 피처(OHLCV) 비중 (1.0=원래). 낮출수록 뉴스 영향 상대적↑
NUM_WEIGHT = 0.6

# 추론 앙상블: base와 (text-only/num-only 혼합) 사이 가중
TEXT_BOOST = 0.6     # 0~1, 높일수록 뉴스 영향↑
ALPHA_TEXT = 0.75    # text-only vs num-only 혼합 비율 (text 쪽 가중)
...
def predict_with_text_boost(model, title, ohlcv, tokenizer=None,
    ...
    """
    p_final = (1-text_boost) * p_base + text_boost * (alpha_text*p_text + (1-alpha_text)*p_num)
    - 재학습 없이 뉴스 영향도를 키우는 추론용 앙상블
    """
    feed_base = build_predict_feed(model, title, ohlcv, tokenizer)
    p_base = float(model.predict(feed_base, verbose=0).squeeze())
    ...
    p_mix = alpha_text * p_text + (1.0 - alpha_text) * p_num
...
model_h5  = check_all_model(model_path_h5, 'model .h5')  # TextVectorization 포함 모델의 h5 저장 미지원 경고는 정상
...
if os.path.exists(model_path) and os.path.exists(tokenizer_path):
    ...
    # 추론용 텍스트 부스트 결과
    boosted = predict_with_text_boost(model, raw_title, ohlcv, tokenizer)
    print(f"[BOOST] base={boosted['p_base']:.4f} | text={boosted['p_text']:.4f} | num={boosted['p_num']:.4f} "
    ...
    model.save(model_path_h5)   # h5는 TextVectorization 포함 모델에서 경고 가능(무시)
